...
public class CloudFoundryServiceWizardIconProviderRegistry {
	...
	public ICloudFoundryServiceWizardIconProvider getIconProvider(String serverRuntimeTypeId) {
		synchronized (this) {
			return providers.get(serverRuntimeTypeId.toLowerCase());
		}
	}
				...
				for (IConfigurationElement config : extension.getConfigurationElements()) {
					...
					if (ELEMENT.equals(config.getName())) {
						try {
							...
							String serverRuntimeTypeId =config.getAttribute("runtimeTypeId");
							
							if(serverRuntimeTypeId != null && provider != null) {
								providers.put(serverRuntimeTypeId, provider);
							} else {
								CloudFoundryPlugin.logError("Invalid server wizard icon extension entry from:"+EXTENSION_POINT);
							}
