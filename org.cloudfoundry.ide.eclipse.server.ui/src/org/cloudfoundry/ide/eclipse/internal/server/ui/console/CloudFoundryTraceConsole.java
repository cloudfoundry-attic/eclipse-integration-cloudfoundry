...
import org.cloudfoundry.ide.eclipse.internal.server.core.trace.TraceType;
...
public class CloudFoundryTraceConsole {
	...
	static final private TraceUIType[] TRACE_TYPES = new TraceUIType[] {
			...
			new TraceUIType(TraceType.GENERAL, SWT.COLOR_BLACK) };

	private final MessageConsole messageConsole;

	private Map<ITraceType, TraceConsoleStream> streams;
	...
	public synchronized void close() {
		if (streams != null) {
			for (Entry<ITraceType, TraceConsoleStream> entry : streams.entrySet()) {
	...
	public synchronized void initialiseStreams() {
		...
		streams = new HashMap<ITraceType, TraceConsoleStream>();
	...
	public synchronized void tail(String message, ITraceType type) {
	...
	protected synchronized void write(String message, ITraceType type) {
	...
	static class TraceUIType {
		...
		private final ITraceType type;

		private TraceUIType(ITraceType type, int swtColour) {
		...
		public ITraceType getTraceType() {
			return type;
		}
	}
}
